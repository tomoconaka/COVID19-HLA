...
wb <- readRDS("couples_step4_WB_withHLA.rds")
sas <- readRDS("couples_step4_SAS_withHLA.rds")

couples <- couples %>% inner_join(p, by=c("f.eid" = "FID"))
couples <- couples %>% filter(!(f.eid %in% c(wb$f.eid, sas$f.eid)))

couples <- couples %>% mutate(POP = case_when(CLUST %in% c(1,3,9,10,11) ~ "EUR",
                                              CLUST %in% c(2,12) ~ "AMR",
                                              CLUST == 4 ~ "EAS",
                                              CLUST == 5 ~ "others",
                                              CLUST == 6 ~ "AFR",
...
tmp <- couples %>% arrange(POP) %>% group_by(uniqueCode) %>%
  mutate(POP_pair = paste0(POP[1],"_",POP[2])) %>% select(uniqueCode, POP_pair) %>% unique()

table(tmp$POP_pair)

saveRDS(couples, file="couples_step4_others_withHLA.rds")

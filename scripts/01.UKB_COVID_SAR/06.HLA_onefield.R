...
#Class 1
tmp_cor <- tmp %>% dplyr::select(HLAAmatch, HLABmatch, HLACmatch)
library(corrplot)
library(Hmisc)
...
res1 <- rma.mv(yi,V)
png("onefield.meta_WB_class1_20230414.png", width=500, height = 400)
forest(res1, atransf=exp, slab = out$HLAgene[1:3])
dev.off()


#Class 2
tmp_cor <- tmp %>% dplyr::select(HLADRB1match, HLADQA1match, HLADQB1match)
library(corrplot)
library(Hmisc)
M <- Hmisc::rcorr(as.matrix(tmp_cor))
col2 = colorRampPalette(c('#005BB6', 'white', '#C70000'))
...
res2 <- rma.mv(yi,V)
png("onefield.meta_WB_class2_20230414.png", width=500, height = 400)
forest(res2, atransf=exp, slab = out$HLAgene[4:6])
dev.off()
save(tab3Mat, out, res1, res2, file = "onefield_WB_20230414.Rdata")
...
res1 <- rma.mv(yi,V)
png("onefield.meta_SAS_class1_20230414.png", width=500, height = 400)
forest(res1, atransf=exp, slab = out$HLAgene[1:3])
dev.off()
...
res2 <- rma.mv(yi,V)
png("onefield.meta_SAS_class2_20230414.png", width=500, height = 400)
forest(res2, atransf=exp, slab = out$HLAgene[4:6])
dev.off()
save(tab3Mat, out, res1, res2, file = "onefield_SAS_2023044.Rdata")
...
##meta HLA-A
load("onefield_WB_20230414.Rdata")
res_WB <- out %>% filter(HLAgene == "HLAAmatch")
load("onefield_SAS_2023044.Rdata")
res_SAS <- out %>% filter(HLAgene == "HLAAmatch")
res_WB <- res_WB %>% mutate(ancestry = "European")
res_SAS <- res_SAS %>% mutate(ancestry = "South Asian")
...
##meta HLA-B
load("onefield_WB_20230414.Rdata")
res_WB <- out %>% filter(HLAgene == "HLABmatch")
load("onefield_SAS_2023044.Rdata")
res_SAS <- out %>% filter(HLAgene == "HLABmatch")
res_WB <- res_WB %>% mutate(ancestry = "European")
res_SAS <- res_SAS %>% mutate(ancestry = "South Asian")
...
##meta HLA-C
load("onefield_WB_20230414.Rdata")
res_WB <- out %>% filter(HLAgene == "HLACmatch")
load("onefield_SAS_2023044.Rdata")
res_SAS <- out %>% filter(HLAgene == "HLACmatch")
res_WB <- res_WB %>% mutate(ancestry = "European")
res_SAS <- res_SAS %>% mutate(ancestry = "South Asian")

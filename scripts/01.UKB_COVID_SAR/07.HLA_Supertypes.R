...
write.csv(tab3Mat, file = "Table_supertypeHLA1_WB.csv")
...
write.table(out, file="supertypeHLA1_regression_WB.tsv", col.names = T, row.names = F, sep="\t", quote = F)
...
saveRDS(M, file="HLA_supertypeHLA1_correlation_WB.rds")
...
res <- rma.mv(yi,V)
png("HLA_supertypeHLA1.meta_WB.png", width=500, height = 400)
forest(res, atransf=exp, slab = out$HLAgene)
dev.off()


save(tab3Mat, out, M, metaresult, file = "supertypeHLA1_WB.Rdata")
...
tableOne <- CreateTableOne(vars = catVars, strata = c("outcome"), data = tmp,
                           factorVars = catVars )
tab3Mat <- print(tableOne, showAllLevels = TRUE, quote=F, noSpaces=TRUE, exact = catVars)
write.csv(tab3Mat, file = "Table_supertypeHLA2_WB.csv")
...
for(i in seq(1, 5)){
  LM <- glm(paste0("outcome ~ ",HLAgene[i]," + age_diff + age_sum + second_sex + f.31.0.0 + PC1dif + PC2dif + PC3dif + PC4dif + PC5dif +PC6dif +
                   PC7dif + PC8dif + PC9dif + PC10dif"), data=tmp, family="binomial")
  out$HLAgene[i] <- HLAgene[i]
  out$beta[i] <- summary(LM)$coefficients[2, 1]
  out$se[i] <- summary(LM)$coefficients[2, 2]
...
write.table(out, file="supertypeHLA2_regression_WB.tsv", col.names = T, row.names = F, sep="\t", quote = F)
...
saveRDS(M, file="HLA_supertypeHLA2_correlation_WB.rds")
...
save(tab3Mat, out, M, metaresult, file = "supertypeHLA2_WB.Rdata")

res <- rma.mv(yi,V)
png("HLA_supertypeHLA2.meta_WB.png", width=500, height = 400)
forest(res, atransf=exp, slab = out$HLAgene)
dev.off()
